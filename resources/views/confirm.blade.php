        ...
        <header class="bg-white dark:bg-gray-800 shadow">
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex justify-between">
                <a href="/archive" class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Konfirmasi Verifikasi TUK - Direktur LSP
                </a>
                                ...
                                <tbody>
                                    @foreach ($all_verifications as $verification)
                                        <tr>
                                            ...
                                            <td><a href="/confirm/{{ $verification['id'] }}">
                                    ...
                                    @foreach ($all_sk as $sk)
                                        <tr>
                                            ...
                                            <td>{{ $sk['link'] }}</td>
                                            <td>{{ $sk['created_at'] }}</td>
                                            <td><a href="{{ Storage::disk('public')->url('tuk-paperless/' . \Carbon\Carbon::parse($sk['created_at'])->format('Y-m-d') . '/' . strtoupper($sk['tuk']) . '/' . $sk['link']) }}" target="_blank">
                                                    <div class="py-2 px-1 bg-green-600 rounded text-center">Lihat File
                                                    </div>
                                                </a>
                                            </td>
                                            <td><a href="/sk/{{ $sk['id'] }}">
                                                    <div class="py-2 px-1 bg-green-600 rounded text-center">Buat SK
                                                    </div>
                                                </a>
                                            </td>
                                        </tr>
