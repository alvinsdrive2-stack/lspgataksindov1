        ...
        foreach ($skemaArray as $index => $skema) {
            ...
            if (!empty($skema)) {
                ...
                for ($i = 1; $i <= $pageCount; $i++) {
                    ...
                    if ($i === 8 && $jabker->jenjang_id > 3 || $jabker->jenjang_id < 4 && $jabker->peralatan === null) {
    ...
    public function createFileMandiri(Request $request)
    {
        ...
        foreach ($additionalData as $key => $data) {
            if (in_array($key, $subklasArray)) {
                // Merge unique additional subklas
                $subklasArray = array_unique(array_merge($subklasArray, $data['subklas']));

                // Merge unique additional skema if available
                if (isset($data['skema'])) {
        ...
        foreach ($subklasArray as $subklas) {
